...
def run_backtest(data):
    ...
    for _, row in bd.iterrows():
        ...
        nav_daily.append({'date': row['date'], 'nav': mkt, 'cyb_hold': cyb_val, 'hldb_hold': hldb_val, 'ratio': ratio, 'position': position})
...
def generate_page():
    ...
    if ratio < BUY_THRESHOLD:
        signal, hold, signal_class, reason = "买入创业板", "创业板指 (399006)", "buy-cyb", f"比值 {ratio*100:.2f}% < 20%，创业板相对低估，建议买入创业板。"
    elif ratio > SELL_THRESHOLD:
        signal, hold, signal_class, reason = "买入红利低波", "红利低波 (H30269)", "buy-hldb", f"比值 {ratio*100:.2f}% > 40%，创业板相对高估，建议切换至红利低波。"
    else:
        if ratio < 0.30:
            signal, hold, signal_class, reason = "建议持有创业板", "创业板指 (399006)", "hold-cyb", f"比值 {ratio*100:.2f}%，处于20%-40%观望区间，偏创业板方向。"
        else:
            signal, hold, signal_class, reason = "建议持有红利低波", "红利低波 (H30269)", "hold-hldb", f"比值 {ratio*100:.2f}%，处于20%-40%观望区间，偏红利低波方向。"
